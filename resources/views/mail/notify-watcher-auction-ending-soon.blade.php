@extends('core::emails.base')

@section('body')
    <table class="email__inner" style=""
           width="100%" cellpadding="40">
        <tbody>
        <tr>
            <td>
                <h1 class="email__headline">An auction you are watching is ending soon.</h1>
                <h2 class="email__subhead">
                    <a href="{{ route('single_ad', ['id' => $listing->id, 'slug' => $listing->slug]) }}">
                        {{ $listing->title }}
                    </a>
                </h2>
                @if ($listing->winning_bid)
                    <p>The current winning bid is {{ Currency::format($listing->winning_bid->bid_amount) }}</p>
                @else
                    No one made a bid yet.
                @endif
            </td>
        </tr>
        </tbody>
    </table>
@endsection
