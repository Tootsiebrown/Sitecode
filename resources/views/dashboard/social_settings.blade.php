            ...
            <div id="page-wrapper">
                ...
                <form action="{{route('save_settings')}}" class="form-horizontal" method="post" enctype="multipart/form-data"> @csrf
                    ...
                    <div id="social_login_settings_wrap">
                        ...
                        <div class="form-group {{ $errors->has('enable_twitter_login')? 'has-error':'' }}">
                            <label class="col-md-4 control-label">@lang('app.enable_disable') </label>
                            <div class="col-md-8">
                                <label for="enable_twitter_login" class="checkbox-inline">
                                    <input type="checkbox" value="1" id="enable_twitter_login" name="enable_twitter_login" {{ get_option('enable_twitter_login') == 1 ? 'checked="checked"': '' }}>
                                    @lang('app.enable_twitter_login')
                                </label>
                            </div>
                        </div>
                        ...
                        <div id="twitter_login_api_wrap" style="display: {{ get_option('enable_twitter_login') == 1 ? 'block' : 'none' }};">
                            <hr />
                            <div class="form-group">
                                <label for="twitter_consumer_key" class="col-sm-4 control-label">@lang('app.twitter') @lang('app.consumer_key')</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="twitter_consumer_key" value="{{ get_option('twitter_consumer_key') }}" name="twitter_consumer_key" placeholder="@lang('app.consumer_key')">
                                </div>
                            </div>

                            <div class="form-group">
                                <label for="twitter_consumer_secret" class="col-sm-4 control-label">@lang('app.twitter') @lang('app.consumer_secret')</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="twitter_consumer_secret" value="{{ get_option('twitter_consumer_secret') }}" name="twitter_consumer_secret" placeholder="@lang('app.consumer_secret')">
                                </div>
                            </div>
...
@section('page-js')
    <script>
        $(document).ready(function(){
            ...
            $('#enable_twitter_login').click(function(){
                if ($(this).prop('checked')){
                    $('#twitter_login_api_wrap').slideDown();
                }else{
                    $('#twitter_login_api_wrap').slideUp();
                }
            });
